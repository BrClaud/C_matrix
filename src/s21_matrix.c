...
//  запонение матрицы - TODO скрыть
// int fill_matrix(matrix_t *A) {
//   int status = 0;
//   if (A && A->matrix) {
...
//       if (A->matrix[i]) {
//         for (int j = 0; j < A->columns; j++) {
//           if (scanf("%lf%c", &(A->matrix[i][j]), &trash) != 2 ||
//               (j == A->columns - 1 && trash != 10) ||
//               (j < A->columns - 1 && trash != ' ')) {
//             printf("n/a");
//             status = 1;
//             break;
